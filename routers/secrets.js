...
router.get("/all", (req, res) => {
  Secret.all(req.user._id).then(secrets => {
    Secret.userRequested(req.user._id).then(requestedSecrets => {
      res.render("allSecrets", { secrets, requestedSecrets });
    });
  });
});
...
router.post("/request", (req, res) => {
  Secret.findByIdAndUpdate(req.body.secretId, {
    $push: { whoRequested: req.user._id }
  }).then(() => {
...
router.post("/accept", (req, res) => {
  Secret.findByIdAndUpdate(req.body.secretId, {
    $pull: { whoRequested: req.body.giveAccessId },
    $push: { hasAccess: req.body.giveAccessId }
  }).then(() => {
